pipeline {
    ...
    stages {
        ...
        stage('Push Docker Image') {
            steps {
                withCredentials([string(credentialsId: 'docker-pass-id', usernameVariable: 'Username', passwordVariable: 'Password')]) {
                    sh(script: "sudo docker login -u ${Username} -p ${Password}")
                }
